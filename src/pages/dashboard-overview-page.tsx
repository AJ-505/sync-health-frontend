...
import { ArrowRight, ChevronDown, Filter, HeartPulse, LogOut, Mail, Search, Sparkles, X } from "lucide-react"
...
function getFollowUpTips(disease: string): string[] {
  const normalized = disease.toLowerCase()

  if (normalized.includes("hypertension") || normalized.includes("blood pressure")) {
    return [
      "Monitor blood pressure regularly over the next 2-4 weeks.",
      "Reduce sodium intake and prioritize hydration.",
      "Schedule a clinical blood pressure review.",
    ]
  }

  if (normalized.includes("diabetes") || normalized.includes("glucose")) {
    return [
      "Track fasting glucose and reduce added sugars.",
      "Prioritize consistent physical activity each week.",
      "Book a follow-up metabolic screening.",
    ]
  }

  if (normalized.includes("cardio") || normalized.includes("heart")) {
    return [
      "Increase moderate cardiovascular activity as tolerated.",
      "Limit smoking and alcohol exposure where applicable.",
      "Schedule a cardiovascular risk check with a clinician.",
    ]
  }

  return [
    "Review your latest health screening metrics.",
    "Maintain a preventive lifestyle plan this month.",
    "Schedule a follow-up with a healthcare professional.",
  ]
}
...
export function DashboardOverviewPage({ members, user, onLogout }: DashboardOverviewPageProps) {
  ...
  const aiEmailRecipients = useMemo(() => {
    if (!aiRiskFilter) return []
    const uniqueEmails = new Set<string>()

    for (const entry of aiRiskFilter.entries) {
      if (!entry.memberId) continue
      const email = memberEmailById.get(entry.memberId)
      if (email) {
  ...
  }, [aiRiskFilter, memberEmailById])

  const aiCategoryEmailLink = useMemo(() => {
    if (!aiRiskFilter || aiEmailRecipients.length === 0) return null

    const tips = getFollowUpTips(aiRiskFilter.disease)
    const subject = `Preventive Health Follow-up: ${aiRiskFilter.disease} Risk Group`
    const bodyLines = [
      "Hello,",
      "",
      `This is a preventive outreach regarding elevated ${aiRiskFilter.disease.toLowerCase()} risk identified in our latest health analytics review.`,
      "",
      "Suggested next steps:",
      ...tips.map((tip) => `- ${tip}`),
      "",
      "Please contact the health team if you need support.",
      "",
      `Generated by Sync Health AI on ${new Date().toLocaleDateString()}.`,
    ]

    return `mailto:${aiEmailRecipients.join(",")}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(bodyLines.join("\n"))}`
  }, [aiRiskFilter, aiEmailRecipients])
                    ...
                    {aiRiskFilter && (
                      aiCategoryEmailLink ? (
                        ...
                        <Button
                          ...
                          title="No matched email recipients found for this AI filter."
                        >
                          <Mail className="size-3.5" />
                          Mail All
                        </Button>
